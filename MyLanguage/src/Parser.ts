import { BinOperatinoNode } from "./AST/BinOperationNode";
import ExpreesionNode from "./AST/ExpressionNode";
...
export default class Parser {
    ...
    parseVariableOrNumber(): ExpreesionNode {
    ...
    parseParentheses(): ExpreesionNode{
    ...
    parseFormula(): ExpreesionNode{
        ...
        while( operator != null){
            ...
            leftNode = new BinOperatinoNode(operator, leftNode, rightNode);
            operator = this.match(tokenTypeList.MINUS, tokenTypeList.PLUS);
        }
        return leftNode;
    }

    parseExpression(): ExpreesionNode {
        ...
        if(assignOperator != null){
            ...
            const binaryNode = new BinOperatinoNode(assignOperator, VariableNode, rightFormulaNode);
            return binaryNode;
        }
        throw new Error(`После переменной ожидается оператор присваивания на позиции ${this.pos}`);
    }

    parseCode(): ExpreesionNode {
    ...
    run(node: ExpreesionNode): any {
        ...
        if(node instanceof BinOperatinoNode){
            switch(node.operator.type.name){
